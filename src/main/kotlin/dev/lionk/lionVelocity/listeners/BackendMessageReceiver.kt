    ...
    fun onReceive(server: RegisteredServer, to: TransferrableObject){
        when (to.objectType){
            ...
            "LionLobby_PlayerTransfer" -> {
                ...
                val withReconnect = to.getBool("support_reconnect")
                if (player.isPresent){
                    if (withReconnect)
                        AutoConnectionManager.addNewConnection(server.serverInfo.name, player.get().uniqueId)
                    if (rs.isPresent){
                        player.get().saveQueueReconnect(rs.get())
                    } else {
                        LionVelocity.instance.logger.error("Server " + to.getString("server") + " wasn't found")
                        LionChat.sendMessageOnChannel("velocity", "command.players.send.no_server".translate(), player.get())
                    }
